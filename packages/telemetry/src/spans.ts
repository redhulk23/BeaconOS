...
export function createAgentSpan(
  ...
  return tracer.startSpan(`agent.${agentName}`, {
    ...
    ...options,
    attributes: {
      "beacon.span_type": "agent",
      "beacon.agent_name": agentName,
      ...options?.attributes,
    },
  });
}
...
export function createToolSpan(
  ...
  return tracer.startSpan(`tool.${toolName}`, {
    ...
    ...options,
    attributes: {
      "beacon.span_type": "tool",
      "beacon.tool_name": toolName,
      ...options?.attributes,
    },
  });
}
...
export function createModelSpan(
  ...
  return tracer.startSpan(`model.${provider}.${model}`, {
    ...
    ...options,
    attributes: {
      "beacon.span_type": "model",
      "beacon.model_provider": provider,
      "beacon.model_name": model,
      ...options?.attributes,
    },
  });
}
...
export function createWorkflowSpan(
  ...
  return tracer.startSpan(`workflow.${workflowName}`, {
    ...
    ...options,
    attributes: {
      "beacon.span_type": "workflow",
      "beacon.workflow_name": workflowName,
      ...options?.attributes,
    },
  });
}
