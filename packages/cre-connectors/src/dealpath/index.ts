...
const MOCK_UNDERWRITING: DealPathUnderwriting = {
  dealId: "DP001",
  purchasePrice: 25_500_000,
  closingCosts: 510_000,
  renovationBudget: 2_000_000,
  stabilizedNoi: 1_785_000,
  goingInCapRate: 0.07,
  exitCapRate: 0.065,
  holdPeriod: 7,
  leveragePercent: 0.65,
  interestRate: 0.055,
  unleveredIrr: 0.092,
  leveredIrr: 0.142,
  equityMultiple: 2.1,
  ...
  items: [
    {
      itemId: "CL001",
      category: "Legal",
      description: "Title search and review",
      status: "completed",
    ...
    },
    {
      itemId: "CL003",
      category: "Financial",
      description: "T-12 operating statement review",
      status: "completed",
      assignedTo: "Analyst",
      dueDate: "2024-06-10",
    },
    ...
    },
    {
      itemId: "CL005",
      category: "Physical",
      description: "Phase I environmental",
      status: "not_started",
    ...
    },
    {
      itemId: "CL007",
      category: "Tenant",
      description: "Estoppel certificates",
      status: "in_progress",
...
export async function dealpathGetDeals(input: {
  stage?: string;
  propertyType?: string;
}): Promise<DealPathDeal[]> {
  log.info(input, "Getting deals from DealPath");
  return MOCK_DEALS.filter((d) => {
    if (input.stage && d.stage !== input.stage) return false;
    if (input.propertyType && d.propertyType !== input.propertyType)
      return false;
    return true;
  });
}

export async function dealpathGetUnderwriting(input: {
  dealId: string;
}): Promise<DealPathUnderwriting> {
  log.info({ dealId: input.dealId }, "Getting underwriting from DealPath");
  return MOCK_UNDERWRITING;
}

export async function dealpathGetChecklist(input: {
  dealId: string;
}): Promise<DealPathChecklist> {
  log.info({ dealId: input.dealId }, "Getting DD checklist from DealPath");
  return MOCK_CHECKLIST;
}

export async function dealpathUpdateDealStage(input: {
  dealId: string;
  stage: string;
  notes?: string;
}): Promise<{ dealId: string; stage: string; status: string }> {
...
export const dealpathTools = [
  {
    ...
    description:
      "Get investment deals from DealPath pipeline by stage or property type",
    inputSchema: {
      type: "object",
      properties: {
        stage: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Get underwriting assumptions and returns for a deal from DealPath",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Get the due diligence checklist and status for a deal from DealPath",
    inputSchema: {
      type: "object",
      properties: { dealId: { type: "string" } },
      required: ["dealId"],
