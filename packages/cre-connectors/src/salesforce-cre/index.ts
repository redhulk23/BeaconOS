...
const MOCK_CONTACTS: SfContact[] = [
  ...
  },
  {
    ...
    firstName: "Lisa",
    lastName: "Park",
    email: "lpark@investco.com",
    company: "InvestCo Capital",
...
const MOCK_DEALS: SfDeal[] = [
  {
    dealId: "SFD001",
    name: "Beacon Tower — Acme Renewal",
  ...
  {
    dealId: "SFD002",
    name: "Harbor View — InvestCo Acquisition",
    ...
    contactId: "SF003",
    notes: "Preliminary interest stage",
  },
];

// --- MCP Tool Functions ---

export async function sfGetContacts(input: {
  type?: string;
  company?: string;
  search?: string;
}): Promise<SfContact[]> {
  ...
  return MOCK_CONTACTS.filter((c) => {
    ...
    if (
      input.company &&
      !c.company.toLowerCase().includes(input.company.toLowerCase())
    )
      return false;
    if (input.search) {
...
export async function sfGetDeals(input: {
  stage?: string;
  dealType?: string;
  contactId?: string;
}): Promise<SfDeal[]> {
...
export async function sfGetPipeline(input: {
  dealType?: string;
}): Promise<SfPipelineSummary> {
  log.info(input, "Getting pipeline summary from Salesforce CRE");
  const deals = input.dealType
    ? MOCK_DEALS.filter((d) => d.dealType === input.dealType)
    : MOCK_DEALS;
...
export async function sfLogActivity(input: {
  type: "call" | "email" | "meeting" | "task" | "note";
  subject: string;
  description?: string;
  contactId?: string;
  dealId?: string;
}): Promise<{ activityId: string; status: string }> {
...
export const salesforceCRETools = [
  {
    ...
    description:
      "Search for contacts in Salesforce CRE CRM by type, company, or name",
    inputSchema: {
      type: "object",
      properties: {
        type: { type: "string" },
  ...
  },
  {
    name: "sf_get_deals",
    description:
      "Get deals from Salesforce CRE pipeline by stage, type, or contact",
    inputSchema: {
      type: "object",
      properties: {
        stage: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Get pipeline summary with stage breakdown from Salesforce CRE",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Log a call, email, meeting, or note activity in Salesforce CRE",
    inputSchema: {
      type: "object",
      properties: {
        type: { type: "string" },
