...
const FAIR_HOUSING_TERMS = [
  ...
  "disability", "familial status", "handicap",
];
...
export function toPassFairHousing(content: string): boolean {
  ...
  for (const term of FAIR_HOUSING_TERMS) {
    if (lower.includes(`exclude ${term}`) || lower.includes(`deny based on ${term}`) || lower.includes(`not ${term}`)) {
      return false;
    }
  }
  return true;
...
export function toHaveAuditLog(logs: Array<{ action: string; resourceType: string }>): boolean {
  return logs.length > 0;
}

export function toHaveDisclaimer(content: string): boolean {
  const lower = content.toLowerCase();
  return lower.includes("disclaimer") || lower.includes("not legal advice") || lower.includes("consult") || lower.includes("for informational purposes");
}
