import type { Property, Unit, Lease, CRETenant, Financial } from "../entities/index.js";

// --- Yardi Mappers (stubs matching typical Yardi Voyager API shapes) ---

export function propertyFromYardi(yardiProperty: Record<string, unknown>): Partial<Property> {
  return {
...
export function leaseFromYardi(yardiLease: Record<string, unknown>): Partial<Lease> {
  return {
...
export function propertyFromMRI(mriProperty: Record<string, unknown>): Partial<Property> {
  return {
...
export function leaseFromMRI(mriLease: Record<string, unknown>): Partial<Lease> {
  return {
...
export function propertyFromCoStar(costarProperty: Record<string, unknown>): Partial<Property> {
  return {
...
function mapYardiPropertyType(type: string): Property["propertyType"] {
  const map: Record<string, Property["propertyType"]> = {
    OFF: "office", RET: "retail", IND: "industrial", MF: "multifamily",
    MIX: "mixed_use", HTL: "hospitality", LND: "land",
  };
...
function mapYardiLeaseType(type: string): Lease["leaseType"] {
  const map: Record<string, Lease["leaseType"]> = {
    GRS: "gross", NET: "net", NNN: "nnn", MOD: "modified_gross", PCT: "percentage",
  };
...
function mapMRIPropertyType(type: string): Property["propertyType"] {
  const map: Record<string, Property["propertyType"]> = {
    ...
    residential: "multifamily", mixed: "mixed_use",
  };
...
function mapMRILeaseType(type: string): Lease["leaseType"] {
  const map: Record<string, Lease["leaseType"]> = {
    gross: "gross", net: "net", nnn: "nnn", modified: "modified_gross",
  };
...
function mapCoStarPropertyType(type: string): Property["propertyType"] {
  const map: Record<string, Property["propertyType"]> = {
    Office: "office", Retail: "retail", Industrial: "industrial",
    Multifamily: "multifamily", Flex: "industrial", Hospitality: "hospitality",
  };
  return map[type] ?? "office";
}
