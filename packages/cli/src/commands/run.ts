...
export const runCommand = new Command("run")
  ...
  .action(
    async (agent: string, options: { input: string; dryRun?: boolean }) => {
      ...
      if (!token) {
        ...
        console.log(
          chalk.dim(
            "For local development, the agent manifest is valid and ready to run.",
          ),
        );
        return;
      }
      ...
      try {
        const response = await fetch(
          `${apiUrl}/api/v1/agents/${metadata.name}/runs`,
          {
            method: "POST",
            headers: {
