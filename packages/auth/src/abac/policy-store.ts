...
export class PolicyStore {
  async getPoliciesForTenant(tenantId: string): Promise<AbacPolicy[]> {
    ...
    const rows = await db
      .select()
      .from(abacPolicies)
      .where(and(eq(abacPolicies.tenantId, tenantId), eq(abacPolicies.status, "active")));
    ...
    return rows.map((row) => ({
      ...
      subjectAttributes: (row.subjectAttributes ?? []) as AbacPolicy["subjectAttributes"],
      resourceAttributes: (row.resourceAttributes ?? []) as AbacPolicy["resourceAttributes"],
      actionAttributes: (row.actionAttributes ?? []) as AbacPolicy["actionAttributes"],
  ...
  async createPolicy(input: {
    ...
    await db.insert(abacPolicies).values({
      ...
      effect: input.effect,
      subjectAttributes: input.subjectAttributes as unknown as Record<string, unknown>,
      resourceAttributes: input.resourceAttributes as unknown as Record<string, unknown>,
      actionAttributes: input.actionAttributes as unknown as Record<string, unknown>,
  ...
  async updatePolicy(id: string, tenantId: string, updates: Partial<{
    name: string;
    description: string;
    effect: "allow" | "deny";
    subjectAttributes: AbacPolicy["subjectAttributes"];
    ...
    if (updates.subjectAttributes !== undefined) setValues.subjectAttributes = updates.subjectAttributes;
    if (updates.resourceAttributes !== undefined) setValues.resourceAttributes = updates.resourceAttributes;
    if (updates.actionAttributes !== undefined) setValues.actionAttributes = updates.actionAttributes;
