...
tenantsRouter.get("/current", requirePermission(Permission.TENANTS_READ), async (c) => {
...
tenantsRouter.patch("/current", requirePermission(Permission.TENANTS_MANAGE), async (c) => {
  const tenantId = c.get("tenantId");
...
tenantsRouter.get("/current/settings", requirePermission(Permission.TENANTS_READ), async (c) => {
  const tenantId = c.get("tenantId");
  const db = getDb();
...
tenantsRouter.put("/current/settings/:key", requirePermission(Permission.TENANTS_MANAGE), async (c) => {
  const tenantId = c.get("tenantId");
  ...
  if (!parsed.success) {
    throw new ValidationError("Invalid setting", { issues: parsed.error.issues });
  }
