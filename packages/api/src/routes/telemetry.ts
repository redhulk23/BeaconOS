...
telemetry.get("/traces", requirePermission(Permission.TELEMETRY_READ), async (c) => {
  ...
  if (startAfter) conditions.push(gte(telemetrySpans.startTime, new Date(startAfter)));
  if (startBefore) conditions.push(lte(telemetrySpans.startTime, new Date(startBefore)));
...
telemetry.get("/traces/:traceId", requirePermission(Permission.TELEMETRY_READ), async (c) => {
  const tenantId = c.get("tenantId");
  ...
  const spans = await db
    .select()
    .from(telemetrySpans)
    .where(and(eq(telemetrySpans.tenantId, tenantId), eq(telemetrySpans.traceId, traceId)))
...
telemetry.get("/spans", requirePermission(Permission.TELEMETRY_READ), async (c) => {
  const tenantId = c.get("tenantId");
  const db = getDb();
  ...
  if (operationName) conditions.push(eq(telemetrySpans.operationName, operationName));
  if (startAfter) conditions.push(gte(telemetrySpans.startTime, new Date(startAfter)));
  if (startBefore) conditions.push(lte(telemetrySpans.startTime, new Date(startBefore)));
